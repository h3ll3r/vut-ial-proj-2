	...
	if (!*RootPtr) {
		*RootPtr = malloc(sizeof(struct tBTNode));
		...
		if ((*RootPtr)->Cont == Content) {
			...
			tBTNodePtr prevNode = *RootPtr;

			while (true) {
				if (prevNode->Cont == Content) {
					return;
				} else if (prevNode->Cont > Content) {
					if (prevNode->LPtr) {
						prevNode = prevNode->LPtr;
					} else {
						prevNode->LPtr = malloc(sizeof(struct tBTNode));
						prevNode->LPtr->Cont = Content;
						prevNode->LPtr->LPtr = NULL;
						prevNode->LPtr->RPtr = NULL;
						return;
					}
				} else if (prevNode->Cont < Content) {
					if (prevNode->RPtr) {
						prevNode = prevNode->RPtr;
					} else {
						prevNode->RPtr = malloc(sizeof(struct tBTNode));
						prevNode->RPtr->Cont = Content;
						prevNode->RPtr->LPtr = NULL;
						prevNode->RPtr->RPtr = NULL;
						return;
					}
				} // prevNode->Cont == Content
			} // while loop
		} // (*RootPtr)->Cont == Content
	} // !*RootPtr
...
**/
	...
	while (!SEmptyP(&sp)) {
		tBTNodePtr node = STopPopP(&sp);
		bool isFirstVisit = STopPopB(&sb);
		if (!isFirstVisit) {
			BTWorkOut(node);
		} else if (isFirstVisit && !node->RPtr) {
			BTWorkOut(node);
		} else if (node->RPtr) {
			SPushP(&sp, node);
			SPushB(&sb, false);
			Leftmost_Postorder(node->RPtr, &sp, &sb);
		}
	}
